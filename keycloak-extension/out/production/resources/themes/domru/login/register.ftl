    ...
    <#elseif section = "form">
        <form id="registrationForm" action="${url.registrationAction}" method="post">
                ...
                <div class="flex flex-col-reverse justify-between sm:flex-row mt-0 xl:mt-2">
                  ...
                  <span class="flex-basis-1/2 ml-0 mb-6 sm:text-sm text-xs sm:ml-5 sm:mb-0"><span class="opacity-50">Нажимая кнопку, Вы соглашаетесь </span><a class="reference reference_hoverable" href="https://dom.ru/policy.pdf" target="_blink">с политикой обработки данных</a></span>
                </div>
        </form>
    <#elseif section = "info" >
        <p>${twoStepAuthType}</p>
