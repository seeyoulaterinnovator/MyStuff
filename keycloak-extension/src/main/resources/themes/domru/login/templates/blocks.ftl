...
<#macro password >
  ...
  <div class="flex flex-col-reverse sm:flex-row">
    <div class="sm:max-w-1/2">
      <@components.field class="mb-4" fieldName="password" label="${msg('password')}" placeholder="${msg('passwordPlaceholder')}" type="password" required=true />
      
      <@components.field class="mb-4" fieldName="password-confirm" label="${msg('passwordConfirm')}" placeholder="${msg('passwordConfirmPlaceholder')}" type="password" required=true />
    </div>
...
<#macro contentHeader mainTitle secondaryTitle="" secondaryHref="" withBorder=false >
  <header class="flex justify-between items-center pb-2 sm:pb-4">
    <h1 
      id="page-title" 
      class="<#if withBorder>border-extra border-b-2 md:border-b-3 xl:border-b-4</#if>"
    >
      <b>
        ${mainTitle}
      </b>
