        ...
        <div>
            <table id="post" style="width:100%" class="attr">
                ...
                <#list posts as post>
                    ...
                    <tr class="${firstRow?then('selected', '')} attr_row">
<#--                        <td id="roleId-${post?index}">${post.roleId}</td>-->
                        <td id="tomsId-${post?index}">${post.tomsId}</td>
...
<script>
    ...
    Array.from(document.getElementsByClassName('attr_row')).forEach(function (el, index) {
        ...
        var tomsId = document.getElementById("tomsId-" + index).textContent;/*el.getAttribute("toms_id");*/
