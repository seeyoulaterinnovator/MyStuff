...
export type ConvertToCase<T extends string, From extends CaseType, To extends CaseType> =
  From extends 'camel'
  ? To extends 'kebab'
  ? CamelToKebab<T>
  : To extends 'snake'
...
export type CamelToKebab<T extends string> = T extends `${infer P1}${infer P2}${infer P3}`
  ? P2 extends Uppercase<P2>
  ? `${Lowercase<P1>}-${Lowercase<P2>}${CamelToKebab<P3>}`
  : `${Lowercase<P1>}${CamelToKebab<`${P2}${P3}`>}`
  : T;

export type CamelToSnake<T extends string> = T extends `${infer P1}${infer P2}${infer P3}`
  ? P2 extends Uppercase<P2>
  ? `${Lowercase<P1>}_${Lowercase<P2>}${CamelToSnake<P3>}`
  : `${Lowercase<P1>}${CamelToSnake<`${P2}${P3}`>}`
  : T;

export type KebabToCamel<T extends string> = T extends `${infer P1}-${infer P2}${infer P3}`
...
export type SnakeToCamel<T extends string> = T extends `${infer P1}_${infer P2}${infer P3}`
