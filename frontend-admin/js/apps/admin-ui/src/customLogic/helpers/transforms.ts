...
export const convertCase = <T extends string, From extends CaseType, To extends CaseType>(
  str: T,
  from: From,
  to: To
): ConvertToCase<T, From, To> => {
  const transformers = {
    toCamel: (input: string) => input.replace(/[-_](.)/g, (_, char) => char.toUpperCase()) as ConvertToCase<T, From, To>,
    fromCamel: (input: string, separator: string) => input.replace(/([a-z])([A-Z])/g, `$1${separator}$2`).toLowerCase() as ConvertToCase<T, From, To>,
    toKebab: (input: string) => input.replace(/_/g, '-') as ConvertToCase<T, From, To>,
    toSnake: (input: string) => input.replace(/-/g, '_') as ConvertToCase<T, From, To>,
  };
