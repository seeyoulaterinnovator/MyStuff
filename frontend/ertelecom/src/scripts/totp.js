...
export default (function() {
  ...
  function highlightCurrentInput(currentInput, index) {
    ...
    if (currentInput) {
      const borderText = '2px solid ';
      const codeLength = codeNumbers.value;

      if (index === 0 || (index === 3 && codeLength == 6)) {
        currentInput.style.borderBottom = borderText + "#2C9F7D";
      }
      else if (index === 1 || (index === 4 && codeLength == 6)) {
        currentInput.style.borderBottom = borderText + "#E01D1D";
      }
      else if (index === 2) {
        currentInput.style.borderBottom = borderText + "#4F4BFE";
      }
  ...
  function highlightInputsWithGreen() {
    inputs.forEach(input => input.style.borderBottom = '2px solid #2C9F7D');
  }
