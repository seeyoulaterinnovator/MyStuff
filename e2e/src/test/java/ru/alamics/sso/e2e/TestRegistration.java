...
@ExtendWith(TestRegistration.WaitOnFailExtension.class)
@Testcontainers
@Slf4j
...
//        ));
    ...
    void test() {
        ...
        var email = "tester@nomail.tld";
        var phone = "89879870000";
        ...
        var logonUrl = Jsoup.parse(logonPage.body().asString()).body().select("#registrationForm").attr("action");

        assertNotNull(logonUrl);
        log.info("Logon URL: {}", logonUrl);

        var registerUrl = given()
...
//                .formParam("submit", "")
        ...
        log.info("reset URL: {}", registerUrl);
